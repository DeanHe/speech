...
public class PeakValleyDetector {
	...
	/**
	 * Detects peaks (calculates local minima and maxima) in the vector
	 * <code>values</code>. The resulting list contains maxima at the first
	 * position and minima at the last one.
	 * 
	 * Maxima and minima maps contain the indice value for a given position and
	 * the value from a corresponding vector.
	 * 
	 ...
	 */
	public static <U> List<Map<U, Double>> peak_detection(List<Double> values,
		...
		// make sure the first peak is not index 0
		if (maxima.containsKey(0)) {
			maxima.remove(0);
		}
		return peaks;
	}
	 ...
	 * position and minima at the last one.
	 * 
	 * Maxima and minima maps contain the position for a given value and the
	 * value itself from a corresponding vector.
	 * 
	 * A point is considered a maximum peak if it has the maximal value, and was
	 * preceded (to the left) by a value lower by <code>delta</code>.
	 * 
	 ...
	 * @return List of maps (maxima and minima pairs) of detected peaks
	 */
	public static List<Map<Integer, Double>> peak_detection(
			List<Double> values, Double delta) {
		List<Integer> indices = new ArrayList<Integer>();
		for (int i = 0; i < values.size(); i++) {
			indices.add(i);
		}

		return PeakValleyDetector.peak_detection(values, delta, indices);
	}
